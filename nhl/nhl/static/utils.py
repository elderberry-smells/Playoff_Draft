    ...
    for name in players:
        if name.r0_picksin == 'True':
            ...
            if jerk0.pick1 == r0_winners[1]['team']:
                games_off = int(r0_winners[1]['games']) - int(jerk0.games1) # how many points lost for game pick
                total1 = 3 + games_off
                jerk_totals[name.username]['round0'] += total1
            else:
                jerk_totals[name.username]['round0'] = 0

            ###   game 2 east (match 2)
            if jerk0.pick2 == r0_winners[2]['team']:
                games_off = int(r0_winners[2]['games']) - int(jerk0.games2) # how many points lost for game pick
                total2 = 3 + games_off
                jerk_totals[name.username]['round0'] += total2
            else:
                jerk_totals[name.username]['round0'] += 0

            ###   game 3 east (match 3)
            if jerk0.pick3 == r0_winners[3]['team']:
                games_off = int(r0_winners[3]['games']) - int(jerk0.games3) # how many points lost for game pick
                total3 = 3 + games_off
                jerk_totals[name.username]['round0'] += total3
            else:
                jerk_totals[name.username]['round0'] += 0

            ###   game 4 east (match 4)
            if jerk0.pick4 == r0_winners[4]['team']:
                games_off = int(r0_winners[4]['games']) - int(jerk0.games4) # how many points lost for game pick
                total4 = 3 + games_off
                jerk_totals[name.username]['round0'] += total4
            else:
                jerk_totals[name.username]['round0'] += 0

            ###   game 1 west (match 5)
            if jerk0.pick5 == r0_winners[5]['team']:
                games_off = int(r0_winners[5]['games']) - int(jerk0.games5) # how many points lost for game pick
                total5 = 3 + games_off
                jerk_totals[name.username]['round0'] += total5
            else:
                jerk_totals[name.username]['round0'] += 0

            ###   game 2 west (match 6)
            if jerk0.pick6 == r0_winners[6]['team']:
                games_off = int(r0_winners[6]['games']) - int(jerk0.games6) # how many points lost for game pick
                total6 = 3 + games_off
                jerk_totals[name.username]['round0'] += total6
            else:
                jerk_totals[name.username]['round0'] += 0

            ###   game 3 west (match 7)
            if jerk0.pick7 == r0_winners[7]['team']:
                games_off = int(r0_winners[7]['games']) - int(jerk0.games7) # how many points lost for game pick
                total7 = 3 + games_off
                jerk_totals[name.username]['round0'] += total7
            else:
                jerk_totals[name.username]['round0'] += 0

            ###   game 4 west (match 8)
            if jerk0.pick8 == r0_winners[8]['team']:
                games_off = int(r0_winners[8]['games']) - int(jerk0.games8) # how many points lost for game pick
                total8 = 3 + games_off
                jerk_totals[name.username]['round0'] += total8
            else:
                jerk_totals[name.username]['round0'] += 0
    ...
    for name in players:
        if name.r1_picksin == 'True':
            ...
            if jerk1.pick1 == r1_winners[1]['team']:
                games_off = int(r1_winners[1]['games']) - int(jerk1.games1)  # how many points lost for game pick
                total = 7 - games_off
            ...
            if jerk1.pick2 == r1_winners[2]['team']:
                games_off = int(r1_winners[2]['games']) - int(jerk1.games2)  # how many points lost for game pick
                total = 7 - games_off
                jerk_totals[name.username]['round1'] += total
            else:
                jerk_totals[name.username]['round1'] += 0
            ...
            if jerk1.pick3 == r1_winners[3]['team']:
                games_off = int(r1_winners[3]['games']) - int(jerk1.games3)  # how many points lost for game pick
                total = 7 - games_off
                jerk_totals[name.username]['round1'] += total
            else:
                jerk_totals[name.username]['round1'] += 0
            ...
            if jerk1.pick4 == r1_winners[4]['team']:
                games_off = int(r1_winners[4]['games']) - int(jerk1.games4)  # how many points lost for game pick
                total = 7 - games_off
                jerk_totals[name.username]['round1'] += total
            else:
                jerk_totals[name.username]['round1'] += 0
            ...
            if jerk1.pick5 == r1_winners[5]['team']:
                games_off = int(r1_winners[5]['games']) - int(jerk1.games5)  # how many points lost for game pick
                total = 7 - games_off
                jerk_totals[name.username]['round1'] += total
            else:
                jerk_totals[name.username]['round1'] += 0
            ...
            if jerk1.pick6 == r1_winners[6]['team']:
                games_off = int(r1_winners[6]['games']) - int(jerk1.games6)  # how many points lost for game pick
                total = 7 - games_off
                jerk_totals[name.username]['round1'] += total
            else:
                jerk_totals[name.username]['round1'] += 0
            ...
            if jerk1.pick7 == r1_winners[7]['team']:
                games_off = int(r1_winners[7]['games']) - int(jerk1.games7)  # how many points lost for game pick
                total = 7 - games_off
                jerk_totals[name.username]['round1'] += total
            else:
                jerk_totals[name.username]['round1'] += 0
            ...
            if jerk1.pick8 == r1_winners[8]['team']:
                games_off = int(r1_winners[8]['games']) - int(jerk1.games8)  # how many points lost for game pick
                total = 7 - games_off
                jerk_totals[name.username]['round1'] += total
            else:
                jerk_totals[name.username]['round1'] += 0
    ...
    for name in players:
        if name.r2_picksin == 'True':
            ...
            if jerk2.pick1 == r2_winners[1]['team']:
                games_off = (int(r2_winners[1]['games']) - int(
            ...
            if jerk2.pick2 == r2_winners[2]['team']:
                games_off = (int(r2_winners[2]['games']) - int(
            ...
            if jerk2.pick3 == r2_winners[3]['team']:
                games_off = (int(r2_winners[3]['games']) - int(
            ...
            if jerk2.pick4 == r2_winners[4]['team']:
                games_off = (int(r2_winners[4]['games']) - int(
    ...
    for name in players:
        if name.r3_picksin == 'True':
            ...
            if jerk3.pick1 == r3_winners[1]['team']:
                games_off = (int(r3_winners[1]['games']) - int(
            ...
            if jerk3.pick2 == r3_winners[2]['team']:
                games_off = (int(r3_winners[2]['games']) - int(
